...
import com.incognito.ent.adapters.PlaceYourOrderAdapter;
...
public class PlaceYourOrderActivity extends AppCompatActivity {
    ...
    private void calculateTotalAmount(RestaurentModel restaurentModel) {
        float subTotalAmount = 0f;

        for(Menu m : restaurentModel.getMenus()) {
            subTotalAmount += m.getPrice() * m.getTotalInCart();
        }

        tvSubtotalAmount.setText("₹"+String.format("%.2f", subTotalAmount));
        if(isDeliveryOn) {
            tvDeliveryChargeAmount.setText("₹"+String.format("%.2f", restaurentModel.getDelivery_charge()));
            subTotalAmount += restaurentModel.getDelivery_charge();
        }
        tvTotalAmount.setText("₹"+String.format("%.2f", subTotalAmount));
    }
    private void onPlaceOrderButtonClick(RestaurentModel restaurentModel) {
        ...
        i.putExtra("RestaurantModel", restaurentModel);
        startActivityForResult(i, 1000);
    }

    private void initRecyclerView(RestaurentModel restaurantModel) {
